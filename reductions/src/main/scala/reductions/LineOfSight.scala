...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    input.zipWithIndex.foreach {
      case (element, 0) => output(0) = 0
      case (element, i) => output(i) = Math.max(element / i, output(i - 1))
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    (from to until - 1).foldLeft(0f)((max, i) => Math.max(input(i) / i, max))
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    for (i <- from to until - 1) {
      if (i == from) output(i) = Math.max(input(i) / i, startingAngle)
